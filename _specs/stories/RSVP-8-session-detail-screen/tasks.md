...
- [x] **M11.8a** Implement `SessionDetailPage` and `SessionDetailPageModel` directly from the
...
- [x] **M11.8b** Bind `SessionDetailPageModel` to the seed `ISessionsClient` / `IRosterClient`: load
  ...
  The going roster collapses to a `GoingPreviewLimit` (4) preview above a `MoreGoingLabel`
  ("+ N more going") row gated by `HasMoreGoing`, while `GoingHeading` keeps the full count.
...
- [x] **M11.8e** (RSVP-8 slice) `Client.Tests`: the page model exposes the wireframe copy; going and
  ...
  and optimistic RSVP intent with failure rollback have passing tests. The collapsed going roster is
  done — `GoingPreview`/`MoreGoingCount`/`HasMoreGoing`/`MoreGoingLabel` plus two page-model tests
  (`Load_SeededMarinaSession_CollapsesGoingRosterToPreviewWithMoreAffordance` asserting the seeded
  "Going · 16" / four-row preview / "+ 12 more going", and `Load_GoingRosterWithinPreviewLimit_…`
  asserting no affordance when the roster fits). The semantic/responsive contract is covered by
  `SessionScreensXamlTests` (informational-icon semantic descriptions, `ScrollView`, typed Font Awesome
