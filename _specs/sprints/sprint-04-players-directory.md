...
> `PlayerProfiles`, while preserving the existing mobile wireframe design.
...
- `src/SouthBaySoccer.Contracts/Players/PlayerSummaryDto.cs` already defines the public row summary
  shape: player-profile id, display name, initials, position, and guest flag. Internal identity ids
  are intentionally excluded because the client navigates by player-profile id.
...
|---|------|-------|----:|------------|-------|
| 1 | Players contract and data-shape inventory | `PLAYERS-0` | 2 | populated DB | Verify the profile table, nullable fields, sort order, guest visibility, and privacy-safe public row shape. |
...
identity ids, payment identifiers, waiver details, or other private profile fields. The directory
includes active, non-deleted `PlayerProfiles`; the player-profile id is the stable public navigation
key used by the MAUI client.
...
- API mode resolves `IPlayersClient` to `ApiPlayersClient`; Seed mode still resolves
  `SeedPlayersClient`.
- The Players tab loads actual database players from `PlayerProfiles`, not seed rows.
