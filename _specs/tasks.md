...
incremental adoption order in [`documentation/architecture.md`](../documentation/architecture.md) Ã‚Â§18.
...
- Work one milestone at a time; each milestone leaves the solution buildable (architecture Ã‚Â§18). Don't mix sample removal with new feature behavior.
- Status: `[ ]` todo Ã‚Â· `[~]` in progress Ã‚Â· `[x]` done.

---

## Delivery strategy Ã¢â‚¬â€ UI-first (current)

The current focus is the **MAUI/XAML client**. Backend milestones **M1Ã¢â‚¬â€œM10** (Domain, Application,
...
wiring is completed in the backend phase. See `design.md` Ã‚Â§12.

## Sequential roadmap (high level)

1. **M0 Foundation** Ã¢â‚¬â€ shared kernel, `BaseEntity`, cross-cutting abstractions, CI.
2. **M1 Persistence & Identity core** Ã¢â‚¬â€ EF Core `DbContext`, audit/soft-delete, ASP.NET Identity stores, migrations.
3. **M2 Functions pipeline** Ã¢â‚¬â€ middleware (correlation/exception/authn/authz), fail-closed classification, ProblemDetails.
4. **M3 WhatsApp Session Authentication** Ã¯Â¿Â½ phone-number challenge verification, JWT/session issuance, refresh rotation, and backend auth authority. Email/password registration, confirm-email, sign-in, and password reset flows are explicitly out of Sprint 02 scope.
5. **M4 Player profiles & waivers** Ã¢â‚¬â€ profiles, guests, emergency contact, waiver accept + gating (PROF, WAIV).
6. **M5 Payments** Ã¢â‚¬â€ Stripe checkout + idempotent webhooks + membership/ledger (PAY).
7. **M6 Seasons, venues & sessions** Ã¢â‚¬â€ seasons, venues, recurring sessions timer (SES).
8. **M7 RSVP & waitlist** Ã¢â‚¬â€ eligibility-gated RSVP, transactional capacity, waitlist promotion (RSVP, CHK).
9. **M8 Teams, matches & stats** Ã¢â‚¬â€ matches, per-match teams, events, ratings, likes, MVP, corrections (TEAM, STAT).
10. **M9 Leaderboards & queries** Ã¢â‚¬â€ season/career read projections, tie-breakers (LEAD).
11. **M10 Notifications & reliability** Ã¢â‚¬â€ outbox dispatcher, SendGrid/Twilio, reminders (NOTIF).
12. **M11 Client integration** Ã¢â‚¬â€ typed API clients, token handling, replace one sample flow, offline live-stats (ADMIN-2), then client move to `src/SouthBaySoccer.Client/`.
13. **M12 Hardening & deploy** Ã¢â‚¬â€ observability, rate limiting, Key Vault/managed identity, deployment topology, controlled migrations.

---

## M0 Ã¢â‚¬â€ Foundation
- [~] **M0.1** Define `BaseEntity`, common value objects, `IClock`, `ICurrentUser`, result/error types. Ã¢â‚¬â€ Current: `BaseEntity`, `IClock`, `ICurrentUser`, and transport `ApiError` exist; common value objects, an Application result type, and real `Domain.Tests` remain. Ã¢â‚¬â€ Stories: INV-10 Ã‚Â· Projects: Domain, Application.
- [~] **M0.2** Add domain enumerations (roles, event types, payment/membership status, RSVP intent status, attendance outcome). Ã¢â‚¬â€ Current: role/payment/RSVP intent enums exist; event types and attendance outcome remain. Ã¢â‚¬â€ Projects: Domain.
- [ ] **M0.3** Establish CI: build backend projects, run all layer-specific test projects, build the
  Windows MAUI TFM, run analyzers and `dotnet format`. Ã¢â‚¬â€ Done when: pipeline green on the skeleton.
- [~] **M0.4** Confirm decisions in design.md Ã‚Â§10. Ã¢â‚¬â€ Current: membership is resolved as monthly membership plus session-specific guest/drop-in eligibility; SMS timing, balancing, and clean-sheet threshold remain. Ã¢â‚¬â€ Output: decisions recorded in design.md and durable conventions mirrored in `.ai/memory/`.

## M1 Ã¢â‚¬â€ Persistence & Identity core
...
## M3 Ã¯Â¿Â½ WhatsApp Session Authentication
...
- [x] **M3.1** `ITokenService` for JWT issue/validate, access-token claims, and signing-key rotation
  via configuration/Key Vault overlap. - Done: `JwtTokenService` issues and validates HMAC JWTs with `kid`, issuer/audience, roles, policies, expiry, and retired-key validation tests. Ã¯Â¿Â½ Stories: AUTH-3.
- [x] **M3.2** `Features/Authentication`: WhatsApp challenge request/verify Application use cases
  with FluentValidation. - Done: request/verify handlers, validators, safe phone masking, challenge/identity/token issuer seams, and Application tests. External return alone does not authenticate. Ã¯Â¿Â½ Stories: AUTH-8, AUTH-3.
- [x] **M3.3** Refresh-token exchange with atomic rotation, reuse detection, and family revocation.
  Done: EF-backed serializable rotation service hashes raw tokens, consumes/replaces active tokens, marks reuse, and revokes token families. Client-side single-flight refresh is implemented in M11.1. Ã¯Â¿Â½ Stories: AUTH-4.
...
## M4 Ã¢â‚¬â€ Player profiles & waivers
...
- [ ] **M5.1** `IPaymentGateway` + Stripe adapter; `Membership`, `PaymentLedger`, `StripeCustomerReference`. Ã¢â‚¬â€ Stories: PAY-1,3,4.
- [ ] **M5.2** `POST /payments/checkout` for monthly membership (server-side session, short-lived URL; no secret keys client-side). Ã¢â‚¬â€ Stories: PAY-1.
- [ ] **M5.3** `POST /webhooks/stripe` `[AllowAnonymous]`: signature verify on raw body, unique event-ID insert, atomic ledger/membership update, duplicate=2xx no-op, out-of-order guard, failure handling. Ã¢â‚¬â€ Stories: PAY-2,6, INV-1.
- [ ] **M5.4** Eligibility projection consumed by RSVP. Ã¢â‚¬â€ Done when: concurrent/duplicate/out-of-order webhook tests pass (Ã‚Â§17).

## M6 Ã¢â‚¬â€ Seasons, venues & sessions
...
- [x] **M11.0a** Add licensed Font Awesome Free Solid and Brands font resources, register
  ...
  Ã¢â‚¬â€ Stories: INV-13, AUTH-7 Ã‚Â· Projects: MAUI client Ã‚Â· Depends on: M11.0.
- [x] **M11.0c** Extend the reusable UI library for the first product-screen wave: add
  ...
  Ã¢â‚¬â€ Stories: SES-6, PROF-5, LEAD-4, STAT-8 Ã‚Â· Projects: MAUI client Ã‚Â· Depends on: M11.0, M11.0a.
- [x] **M11.0b** Add seed-data providers in `SouthBaySoccer/SeedData/` implementing the client service
  ...
  Release-guarded and carry no real personal data. Ã¢â‚¬â€ Stories: UI-first phase (design.md Ã‚Â§12) Ã‚Â· Projects:
  MAUI client Ã‚Â· Depends on: M11.0.
- [~] **M11.1** `Contracts`-based typed API services + `HttpClient` pipeline (`CorrelationIdHandler`Ã¢â€ â€™`AuthenticationHandler`Ã¢â€ â€™`ApiExceptionHandler`); secure token storage. Ã¢â‚¬â€ Current: typed authentication client and secure token storage are implemented for AUTH-7/8; the shared correlation/authentication/exception handler pipeline remains. Ã¢â‚¬â€ Stories: AUTH-4, NFR-Security.
- [ ] **M11.2** Replace one MAUI sample repository flow with a typed API client (proves the path). Ã¢â‚¬â€ Stories: PROF-1.
- [x] **M11.3a** Implement `WelcomeBackPage` and `WelcomeBackPageModel` directly from the first
  ...
  Ã¢â‚¬â€ Stories: AUTH-7, INV-13 Ã‚Â· Projects: MAUI client Ã‚Â· Depends on: M11.0a.
- [x] **M11.3b** Add typed Pickup Pal configuration, external launcher abstractions, international
  ...
  Ã¢â‚¬â€ Stories: AUTH-8, AUTH-9 Ã‚Â· Projects: MAUI client Ã‚Â· Depends on: M11.1, M11.3a.
- [~] **M11.3c** Implement the WhatsApp one-time challenge client flow and approved deep-link
  ...
  Ã¢â‚¬â€ Stories: AUTH-8, AUTH-3, AUTH-4 Ã‚Â· Projects: Contracts, Functions, Application, MAUI client
  Ã‚Â· Depends on: M3.4, M11.1, M11.3b.
- [~] **M11.3d** Add `Client.Tests` for Welcome Back startup routing, validation, single-submit,
  ...
  Ã¢â‚¬â€ Stories: AUTH-7, AUTH-8, AUTH-9, INV-13 Ã‚Â· Depends on: M11.3c.
- [x] **M11.3e** Implement the first-wave product screens using the per-story slices in
  `stories/`: SES-6 Ã¢â€ â€™ RSVP-8, STAT-7 Ã¢â€ â€™ STAT-8, and PROF-5/LEAD-4 in parallel after M11.0b.
  Integrate shared Shell routes after the owning pages exist.
  Ã¢â‚¬â€ Stories: PROF-5, SES-6, RSVP-8, LEAD-4, STAT-7, STAT-8 Ã‚Â· Depends on: M11.0b, M11.0c.
- [ ] **M11.4** Offline live-stats with idempotency-keyed queue + sync. Ã¢â‚¬â€ Stories: ADMIN-2.
- [ ] **M11.5** Move `SouthBaySoccer/` Ã¢â€ â€™ `src/SouthBaySoccer.Client/` as a dedicated change (no feature work mixed in). Ã¢â‚¬â€ architecture Ã‚Â§5/Ã‚Â§18.
- [ ] **M11.6** Admin create-session screen against seed state, publishing into the Sessions feed for RSVP. - Stories: ADMIN-4.

## M12 Ã¢â‚¬â€ Hardening & deploy
- [ ] **M12.1** Application Insights traces/metrics; correlation across client/Functions/providers. Ã¢â‚¬â€ NFR-Observability.
- [ ] **M12.2** Key Vault + managed identity for secrets and SQL; least-privilege identities. Ã¢â‚¬â€ architecture Ã‚Â§8.
- [ ] **M12.3** Rate limiting at API Management/Front Door; deployment topology (Flex Consumption for .NET 10 Linux). Ã¢â‚¬â€ architecture Ã‚Â§16.
- [ ] **M12.4** Controlled migration deployment step; synthetic/health checks; dead-letter alerting.
- [ ] **M12.5** Full Ã‚Â§17 high-risk suite green before release.
...
```
M0 Ã¢â€ â€™ M1 Ã¢â€ â€™ M2 Ã¢â€ â€™ M3 Ã¢â€ â€™ M4 Ã¢â€ â€™ M5 Ã¢â€Â
                     M4 Ã¢â€ â€™ M6 Ã¢â€ â€™ M7 (needs M5 eligibility/payment infrastructure)
                                M7 Ã¢â€ â€™ M8 Ã¢â€ â€™ M9
                     M4 Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€ â€™ M8.6 (profile stat merge)
                     M1 Ã¢â€ â€™ M10
M3..M9 Ã¢â€ â€™ M11 Ã¢â€ â€™ M12
```
