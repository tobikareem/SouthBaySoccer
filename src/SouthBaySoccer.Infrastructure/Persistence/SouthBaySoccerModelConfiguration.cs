...
internal static class SouthBaySoccerModelConfiguration
{
    ...
    private static void ConfigureOperations(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<IdempotencyKey>(b => { ConfigureBase(b, "IdempotencyKeys", false); b.Property(x => x.OperationName).HasMaxLength(128).IsRequired(); b.Property(x => x.Key).HasMaxLength(160).IsRequired(); b.Property(x => x.RequestHash).HasMaxLength(256).IsRequired(); b.Property(x => x.ResponseBodyHash).HasMaxLength(256); b.HasIndex(x => new { x.IdentityUserId, x.OperationName, x.Key }).IsUnique(); });
