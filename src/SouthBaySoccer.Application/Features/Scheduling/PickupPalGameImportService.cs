...
/// <summary>
/// Upserts Pickup Pal games as sessions. This is the single code path for mirroring a game into
/// our database: the active-games import feeds it the whole feed, and the RSVP roster sync feeds
/// it the one game it just changed, so both produce identical sessions, snapshots, participants,
/// and profiles.
/// </summary>
public interface IPickupPalGameImportService
{
    /// <summary>
    /// Creates or updates the session, snapshot, participant rows, and player profiles for each
    /// game. Changes are tracked but <b>not saved</b>; the caller owns the commit so it can batch
    /// the upsert with its own writes.
    /// </summary>
    Task<PickupPalImportResult> UpsertAsync(
        IReadOnlyList<PickupPalGame> games,
        CancellationToken cancellationToken = default);
}

/// <summary>
/// Pickup Pal is the source of truth: a game that matches a session it previously created (by
/// snapshot or occurrence key) adopts and overwrites that session; otherwise a new session is
/// created. A manually-created session is never adopted just because it shares a start time. Each
/// game's sanitized payload and participant roster are persisted alongside the session, and active
/// games with capacity are published.
/// </summary>
public sealed class PickupPalGameImportService(
    ...
    IClock clock) : IPickupPalGameImportService
{
    ...
    public async Task<PickupPalImportResult> UpsertAsync(
        IReadOnlyList<PickupPalGame> games,
        CancellationToken cancellationToken = default)
    {
        if (games.Count == 0)
    ...
    private static string BuildOccurrenceKey(string gameId) => PickupPalOccurrenceKey.Build(gameId);
