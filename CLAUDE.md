...
**Subagents — `.claude/agents/`**: `dotnet-code-reviewer` and `maui-xaml-reviewer` (read-only
reviewers), `test-writer` (drafts xUnit tests), `football-domain-modeler` (models the soccer domain).
**Claude commands — `.claude/commands/`**: `/code-review`, `/create-lessons`, and
`/create-agent-memory`.
**Codex workflow**: `.agents/skills/source-command-code-review/SKILL.md` handles code-review
requests automatically; `.codex/prompts/` contains `/create-lessons` and `/create-agent-memory`.
...
2. **Use subagents** to keep the main context clean: offload exploratory research (reading large
   ...
   `/code-review` skill or a reviewer subagent) after every implementation — not just when asked.
